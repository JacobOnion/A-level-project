...
public class ChasingEnemy : Enemy
{
    ...
    private void Spawner()
    {
        if (health <= 0 && size == 3)
        {
            Instantiate(splitter1, new Vector3(transform.position.x + 0.25f, transform.position.y, 0), transform.rotation);
            Instantiate(splitter1, new Vector3(transform.position.x - 0.25f, transform.position.y, 0), transform.rotation);
        }
        else if (health <= 0 && size == 2)
        {
            Instantiate(splitter2, new Vector3(transform.position.x + 0.25f, transform.position.y, 0), transform.rotation);
            Instantiate(splitter2, new Vector3(transform.position.x - 0.25f, transform.position.y, 0), transform.rotation);
        }
    }
